...
{
    my $iterator = $helper->schema->resultset('TimeTable')->arrivals();
...

{
    ...
    is($timetable->flight->id, 2, 'got flight id');
    is($timetable->flight->code, 332, 'for flight code');
    is($timetable->flight->airline->iata, 'NN', 'got airline iate code');
    is($timetable->flight->departure_airport->city->name, 'Barcelona');
    is($timetable->flight->destination_airport->city->name, 'Moscow');
    is($timetable->departure_time, '14:00:00', 'got time of departure');
    is($timetable->arrival_time, '20:00:00', 'got time af arrival');
